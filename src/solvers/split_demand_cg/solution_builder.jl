"""
    build_sd_demand_solution(instance, selected, obj_val, status, solve_time) → DARPSolution

Convert a vector of selected `SplitDemandRoute`s into a `DARPSolution`.
Pads with idle routes for unused vehicle slots up to K.
"""
function build_sd_demand_solution(
    instance   :: DARPInstance,
    selected   :: Vector{SplitDemandRoute},
    obj_val    :: Float64,
    status     :: Symbol,
    solve_time :: Float64,
    ...
    return DARPSolution(instance, routes, obj_val, is_feas,
                        "SplitDemandCGSolver", solve_time, status,
