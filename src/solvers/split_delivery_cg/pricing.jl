"""
    solve_nocap_pricing(instance, duals; detour_factor, rc_tolerance, max_routes, time_limit)
        → Vector{SplitDeliveryNoCapRoute}

SPPRC pricing subproblem for the uncapped split-delivery CG model.

Vehicle capacity is not enforced. At every pickup i the route boards D_i units
(full demand), so the reduced-cost contribution is −D_i · π_i.
Returned routes are sorted ascending by reduced cost.
"""
function solve_nocap_pricing(
    instance      :: DARPInstance,
    duals         :: SplitDeliveryNoCapDuals;
    detour_factor :: Float64 = Inf,
    rc_tolerance  :: Float64 = -1e-6,
    max_routes    :: Int     = 10,
    time_limit    :: Float64 = 60.0
) :: Vector{SplitDeliveryNoCapRoute}
    n      = instance.n
    T      = instance.T
    L      = instance.L
    N      = 2 * n + 2
    origin = 1
    dest   = N

    pp = _build_sd_preprocess(instance)
    tw_s        = pp.tw_s
    tw_e        = pp.tw_e
    svc_time    = pp.svc_time
    is_pickup   = pp.is_pickup
    is_dropoff  = pp.is_dropoff
    request_of  = pp.request_of
    direct_time = pp.direct_time
    D           = pp.D

    tt = instance.travel_time
    td = instance.travel_distance

    Labels    = [SplitDeliveryNoCapLabel[] for _ in 1:N]
    completed = SplitDeliveryNoCapLabel[]

    root = SplitDeliveryNoCapLabel(
        origin, 0.0, tw_s[origin],
        Int[], Dict{Int,Float64}(),
        [origin], 0.0
    )
    push!(Labels[origin], root)
    open_stack = SplitDeliveryNoCapLabel[root]

    t_start = time()

    while !isempty(open_stack)
        time() - t_start > time_limit && break
        label = pop!(open_stack)   # LIFO = DFS
        cur   = label.node

        for j in 1:N
            j == origin && continue
            j == cur    && continue
            j in label.path && continue
            cur == dest && continue

            # Dropoff only allowed if corresponding request is onboard
            if is_dropoff[j]
                request_of[j] ∉ label.onboard && continue
            end

            arr_j = label.time + svc_time[cur] + tt[cur, j]
            t_j   = max(tw_s[j], arr_j)
            t_j > tw_e[j] + 1e-9 && continue

            new_rc         = label.rc + tt[cur, j]
            new_onboard    = copy(label.onboard)
            new_ride_start = copy(label.ride_start)

            if is_pickup[j]
                req_j = request_of[j]
                # Uncapped: always board D_i units; RC contribution = −D_i · π_i
                new_rc -= D[req_j] * duals.pi[req_j]
                push!(new_onboard, req_j)
                sort!(new_onboard)
                new_ride_start[req_j] = t_j
            elseif is_dropoff[j]
                req_j = request_of[j]
                filter!(r -> r != req_j, new_onboard)
                delete!(new_ride_start, req_j)
            end

            # Ride-time feasibility for all onboard passengers
            feasible = true
            for req_i in label.onboard
                elapsed = t_j - label.ride_start[req_i] - svc_time[pp.pickup_julia[req_i]]
                if elapsed > L + 1e-9 ||
                   (isfinite(detour_factor) && elapsed > detour_factor * direct_time[req_i] + 1e-9)
                    feasible = false; break
                end
            end
            !feasible && continue

            new_dist  = label.distance + td[cur, j]
            new_path  = vcat(label.path, j)
            new_label = SplitDeliveryNoCapLabel(
                j, new_rc, t_j,
                new_onboard, new_ride_start,
                new_path, new_dist
            )

            if j == dest
                isempty(new_onboard) || continue
                t_j - tw_s[origin] > T + 1e-9 && continue
                push!(completed, new_label)
                continue
            end

            if !_sd_is_dominated(new_label, Labels[j])
                _sd_remove_dominated!(Labels[j], new_label)
                push!(Labels[j], new_label)
                push!(open_stack, new_label)
            end
        end
    end

    results = SplitDeliveryNoCapRoute[]
    for lbl in completed
        lbl.rc >= rc_tolerance && continue
        push!(results, _sd_label_to_route(lbl, instance, n, N, pp))
    end

    sort!(results, by = r -> r.reduced_cost)
    return results[1:min(max_routes, length(results))]
end

# ── Internal helpers ───────────────────────────────────────────────────────────

function _build_sd_preprocess(instance::DARPInstance)
    n = instance.n
    N = 2 * n + 2

    tw_s        = zeros(Float64, N)
    tw_e        = zeros(Float64, N)
    svc_time    = zeros(Float64, N)
    is_pickup   = falses(N)
    is_dropoff  = falses(N)
    request_of  = zeros(Int, N)
    pickup_julia = zeros(Int, n)
    direct_time  = zeros(Float64, n)
    D            = zeros(Int, n)

    tw_s[1]     = instance.depot_origin.tw_start
    tw_e[1]     = instance.depot_origin.tw_end
    svc_time[1] = instance.depot_origin.service_time

    for i in 1:n
        pi = i + 1
        di = n + i + 1
        pu = instance.nodes[i]
        dr = instance.nodes[n + i]

        tw_s[pi]       = pu.tw_start;  tw_e[pi]       = pu.tw_end
        svc_time[pi]   = pu.service_time
        is_pickup[pi]  = true
        request_of[pi] = i
        pickup_julia[i] = pi
        D[i]           = pu.load

        tw_s[di]        = dr.tw_start;  tw_e[di]       = dr.tw_end
        svc_time[di]    = dr.service_time
        is_dropoff[di]  = true
        request_of[di]  = i

        direct_time[i] = instance.travel_time[pi, di]
    end

    tw_s[N]     = instance.depot_destination.tw_start
    tw_e[N]     = instance.depot_destination.tw_end
    svc_time[N] = instance.depot_destination.service_time

    return (; tw_s, tw_e, svc_time, is_pickup, is_dropoff,
              request_of, pickup_julia, direct_time, D)
end

# Dominance (uncapped capacity):
#   A dominates B at the same node if all hold:
#     1. A.rc   ≤ B.rc                          (accumulated reduced cost)
#     2. A.time ≤ B.time                         (more future TW slack)
#     3. keys(A.onboard) ⊆ keys(B.onboard)      (fewer active ride-time constraints)
#     4. For each i in A.onboard:
#          elapsed ride time of A ≤ elapsed ride time of B   (more ride-time budget)
#
# Conditions dropped vs. the capacity-constrained version:
#   - A.load ≤ B.load  — removed; load is not a resource without a capacity bound.
#   - A.onboard[i] ≥ B.onboard[i]  — removed; α_i = D_i is fixed and identical for
#     every label that visits pickup i, so this condition is always trivially equal.
function _sd_dominates(A::SplitDeliveryNoCapLabel, B::SplitDeliveryNoCapLabel)
    A.rc   > B.rc   + 1e-9 && return false
    A.time > B.time + 1e-9 && return false

    for r in A.onboard
        r ∉ B.onboard && return false
    end

    for r in A.onboard
        # svc_time at same node cancels on both sides of the comparison
        A.time - A.ride_start[r] > B.time - B.ride_start[r] + 1e-9 && return false
    end

    return true
end

function _sd_is_dominated(lbl::SplitDeliveryNoCapLabel, existing::Vector{SplitDeliveryNoCapLabel})
    for e in existing
        _sd_dominates(e, lbl) && return true
    end
    return false
end

function _sd_remove_dominated!(existing::Vector{SplitDeliveryNoCapLabel}, lbl::SplitDeliveryNoCapLabel)
    filter!(e -> !_sd_dominates(lbl, e), existing)
end

function _sd_label_to_route(
    lbl      :: SplitDeliveryNoCapLabel,
    instance :: DARPInstance,
    n        :: Int,
    N        :: Int,
    pp       :: NamedTuple
) :: SplitDeliveryNoCapRoute
    julia_seq   = lbl.path[2:end-1]
    cordeau_seq = [j - 1 for j in julia_seq]
    request_ids = sort(unique([j - 1 for j in julia_seq if 2 <= j <= n + 1]))

    # With uncapped capacity alpha_i = D_i for every visited request
    alpha = Dict(i => pp.D[i] for i in request_ids)

    svc_times = _sd_replay_service_times(julia_seq, instance, pp.svc_time, n, N)
    loads_v   = _sd_replay_loads(julia_seq, alpha, n)

    ride_times = Float64[]
    for (idx_pu, j) in enumerate(julia_seq)
        2 <= j <= n + 1 || continue
        req_id = j - 1
        di_idx = findfirst(==(n + req_id + 1), julia_seq)
        di_idx === nothing && continue
        rt = svc_times[di_idx] - svc_times[idx_pu] - instance.nodes[req_id].service_time
        push!(ride_times, rt)
    end

    return SplitDeliveryNoCapRoute(
        cordeau_seq, julia_seq, request_ids,
        alpha,
        lbl.distance,
        lbl.rc,
        svc_times, loads_v, ride_times,
        lbl.distance,
        lbl.time - instance.depot_origin.tw_start
    )
end

function _sd_replay_service_times(
    julia_seq :: Vector{Int},
    instance  :: DARPInstance,
    svc_time  :: Vector{Float64},
    n         :: Int,
    N         :: Int
) :: Vector{Float64}
    tt       = instance.travel_time
    cur_time = instance.depot_origin.tw_start
    prev     = 1
    times    = Float64[]
    for j in julia_seq
        arr   = cur_time + svc_time[prev] + tt[prev, j]
        t_j   = max(_sd_tw_start(j, instance, n, N), arr)
        push!(times, t_j)
        cur_time = t_j
        prev     = j
    end
    return times
end

function _sd_tw_start(j::Int, instance::DARPInstance, n::Int, N::Int)::Float64
    j == 1          && return instance.depot_origin.tw_start
    j == N          && return instance.depot_destination.tw_start
    2 <= j <= n + 1   && return instance.nodes[j - 1].tw_start
    n+2 <= j <= 2n+1  && return instance.nodes[j - 1].tw_start
    return 0.0
end

function _sd_replay_loads(julia_seq::Vector{Int}, alpha::Dict{Int,Int}, n::Int)::Vector{Int}
    load  = 0
    loads = Int[]
    for j in julia_seq
        if 2 <= j <= n + 1
            load += get(alpha, j - 1, 0)
        elseif n + 2 <= j <= 2*n + 1
            load -= get(alpha, j - n - 1, 0)
        end
        push!(loads, load)
    end
    return loads
end
