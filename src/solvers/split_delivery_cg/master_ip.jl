"""
    solve_sd_master_ip(instance, pool, env, time_limit_sec, mip_gap, verbose)
        → (status::Symbol, obj::Float64, selected::Vector{SplitDeliveryNoCapRoute})

Binary covering MIP over the final split-delivery column pool.
Constraint: Σ_{r:i∈r} α_{ir} · λ_r ≥ D_i  (integer coefficients α_{ir}).
No fleet constraint.
"""
function solve_sd_master_ip(
    ...
    mip_gap        :: Float64,
    verbose        :: Bool
) :: Tuple{Symbol, Float64, Vector{SplitDeliveryNoCapRoute}}
    n = instance.n
    ...
    if primal_st != JuMP.MOI.FEASIBLE_POINT
        ...
        return status, Inf, SplitDeliveryNoCapRoute[]
    end
