...
#SBATCH --array=1-20
#SBATCH --output=experiments/split_demand/slurm_logs/%A_%a.out
#SBATCH --error=experiments/split_demand/slurm_logs/%A_%a.err
...
#   sbatch scripts/sbatch_split_demand.sh
#
# Override settings:
#   DARP_TIME_LIMIT=600 sbatch scripts/sbatch_split_demand.sh

# ── Solver + n-size lookup ────────────────────────────────────────────────────
...
BASE_OUTDIR="${PROJECT_ROOT}/experiments/split_demand/${SLURM_ARRAY_JOB_ID}"

# ── Header ────────────────────────────────────────────────────────────────────

echo "=========================================="
echo "DARP Split-Demand Benchmark"
